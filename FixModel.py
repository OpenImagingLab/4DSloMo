...
class FixPipeline(BasePipeline):
    ...
    def from_model_manager(model_manager: ModelManager, torch_dtype=None, device=None):
        ...
        pipe = FixPipeline(device=device, torch_dtype=torch_dtype)
    ...
    def encode_image_condition(self, image, inter_images, num_frames, height, width,cam_id):
        image = self.preprocess_image(image.resize((width, height))).to(self.device)
        clip_context = self.image_encoder.encode_image([image])
        ...
        for idx in range(1,len(inter_images)):
            inter_image = self.preprocess_image(inter_images[idx].resize((width, height))).to(self.device)
            vae_input[:,idx,...] = inter_image
        y = self.vae.encode([vae_input.to(dtype=self.torch_dtype, device=self.device)], device=self.device)[0]
        y = torch.concat([msk, y])
        y = y.unsqueeze(0)
        clip_context = clip_context.to(dtype=self.torch_dtype, device=self.device)
    ...
    def __call__(
        ...
        if input_image is not None and self.image_encoder is not None:
            ...
            #     inter_frame.append(input_video[frame_id])

            image_emb = self.encode_image_condition(input_image, input_video, num_frames, height, width, cam_id)
        else:
            image_emb = {}
